...
function agregarRep(oid){const o=state.ordenes.find(o=>o.id===oid);if(!o)return;guardarRepEnMemoria(oid);o.reparaciones.push({desc:'',precio:0});abrirOrden(oid)}
...
function guardarRepEnMemoria(oid){const o=state.ordenes.find(o=>o.id===oid);if(!o)return;o.reparaciones=(o.reparaciones||[]).map((_,i)=>({desc:document.getElementById(`rep-d-${oid}-${i}`)?.value||'',precio:parseFloat(document.getElementById(`rep-p-${oid}-${i}`)?.value)||0}))}
