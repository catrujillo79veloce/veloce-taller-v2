...
function fidelidadCliente(uuid){
  if(!uuid)return{sellos:0,canjeadas:0,ganadas:0,disponibles:0,progreso:0,faltan:SELLOS_META};
  const cli=state.clientes.find(c=>c._uuid===uuid);
  const sellos=state.ordenes.filter(o=>o._clienteUuid===uuid&&(o.status==='done'||o.status==='delivered')&&ordenCalificaFidelidad(o)).length;
...
function fidelidadHTML(uuid,opts){
  ...
  if(f.disponibles>0){
    return `<div class="fidelidad-box fidelidad-premio"><span>🎁 <strong>${f.disponibles} alistada${f.disponibles>1?'s':''} GRATIS disponible${f.disponibles>1?'s':''}</strong></span>${opts.canjear?`<button class="btn btn-sm" onclick="event.stopPropagation();canjearAlistada('${jsStr(uuid)}')">Canjear</button>`:''}</div>`;
  }
