...
const APP_VERSION='v23'; // debe coincidir con la versión del service worker (sw.js)
...
let state = { clientes: [], ordenes: [], mecanicos: ['Carlos','Andrés','Juan'], googleReviewUrl: '', nextId: 1001, consignaciones: [] };
...
// Comisión por mecánico (mensual) según la mano de obra acumulada del mes (SKU 17598)
const COMISION_ESCALONES=[{min:5000000,com:100000},{min:6000000,com:150000},{min:7000000,com:200000}];
function comisionTaller(manoObra){let c=0;for(const t of COMISION_ESCALONES)if(manoObra>=t.min)c=t.com;return c}
function proximoEscalonComision(manoObra){return COMISION_ESCALONES.find(t=>manoObra<t.min)||null}
// ===== Programa de fidelidad: cada 5 servicios (mantenimiento/alistada) = 1 alistada gratis =====
const SELLOS_META=5;
function ordenCalificaFidelidad(o){return (o.tiposTrabajo||[]).some(t=>{const s=String(t||'').trim().toLowerCase();return s.startsWith('mantenimiento')||s.startsWith('alistada')})}
function fidelidadCliente(uuid){
  if(!uuid)return{sellos:0,redenciones:0,ganadas:0,disponibles:0,progreso:0,faltan:SELLOS_META};
  const cli=state.clientes.find(c=>c._uuid===uuid);
  const mias=state.ordenes.filter(o=>o._clienteUuid===uuid);
  // Sellos: servicios terminados que califican y que NO son la alistada de premio
  const sellos=mias.filter(o=>(o.status==='done'||o.status==='delivered')&&ordenCalificaFidelidad(o)&&!o.esPremioFidelidad).length;
  // Redenciones: órdenes marcadas como premio + contador legado (canjes viejos antes de este modelo)
  const premiosOrden=mias.filter(o=>o.esPremioFidelidad).length;
  const canjeadasLegado=cli?cli.alistadasCanjeadas||0:0;
  const redenciones=premiosOrden+canjeadasLegado;
  const ganadas=Math.floor(sellos/SELLOS_META);
  const disponibles=Math.max(0,ganadas-redenciones);
  const progreso=redenciones>ganadas?0:sellos%SELLOS_META;
  return {sellos,redenciones,ganadas,disponibles,progreso,faltan:disponibles>0?0:(SELLOS_META-progreso)};
}
...
function fidelidadHTML(uuid,opts){
  ...
  // Contexto de orden: permite marcar/quitar esta orden como la alistada de premio
  if(opts.orden!=null){
    const o=state.ordenes.find(x=>x.id===opts.orden);
    if(o&&o.esPremioFidelidad){
      return `<div class="fidelidad-box fidelidad-premio"><span>🎁 <strong>Esta orden es la ALISTADA GRATIS (no suma sello)</strong></span><button class="btn btn-sm" onclick="event.stopPropagation();quitarPremioOrden(${o.id})">Quitar</button></div>`;
    }
    if(f.disponibles>0){
      return `<div class="fidelidad-box fidelidad-premio"><span>🎁 <strong>${f.disponibles} alistada${f.disponibles>1?'s':''} GRATIS disponible${f.disponibles>1?'s':''}</strong></span><button class="btn btn-sm" onclick="event.stopPropagation();aplicarPremioOrden(${o.id})">Aplicar a esta orden</button></div>`;
    }
  }
  if(f.disponibles>0){
    return `<div class="fidelidad-box fidelidad-premio"><span>🎁 <strong>${f.disponibles} alistada${f.disponibles>1?'s':''} GRATIS disponible${f.disponibles>1?'s':''}</strong></span><span class="meta">Aplícala en la orden</span></div>`;
  }
...
function refrescarFidelidadUI(uuid){
  const mf=document.getElementById('modal-fidelidad');
  if(mf&&_ordenAbierta!=null){const o=state.ordenes.find(x=>x.id===_ordenAbierta);if(o)mf.innerHTML=fidelidadHTML(o._clienteUuid,{orden:o.id})}
...
let _premioBusy=false;
async function aplicarPremioOrden(oid){
  if(_premioBusy)return;
  const o=state.ordenes.find(x=>x.id===oid);if(!o)return;
  if(fidelidadCliente(o._clienteUuid).disponibles<=0){toast('No hay alistada gratis disponible','error');return}
  if(!confirm('¿Marcar esta orden como la ALISTADA GRATIS de fidelidad? No sumará sello. Recuerda dejar la Alistada en $0.'))return;
  _premioBusy=true;
  try{
    await window.db.updateOrden(oid,{esPremioFidelidad:true});
    o.esPremioFidelidad=true;
    toast('🎁 Alistada gratis aplicada','success');
    refrescarFidelidadUI(o._clienteUuid);
  }catch(err){toast('Error: '+err.message,'error')}
  finally{_premioBusy=false}
}
async function quitarPremioOrden(oid){
  const o=state.ordenes.find(x=>x.id===oid);if(!o)return;
  try{
    await window.db.updateOrden(oid,{esPremioFidelidad:false});
    o.esPremioFidelidad=false;
    toast('Premio de fidelidad quitado','info');
    refrescarFidelidadUI(o._clienteUuid);
  }catch(err){toast('Error: '+err.message,'error')}
}
...
function buildMensajeIngreso(o){
  ...
  if(o.esPremioFidelidad)msg+=`\n🎁 *¡Esta alistada va GRATIS por tu fidelidad!* Gracias por seguir rodando con Veloce 🚲\n`;
...
function buildMensajeCliente(o){
  ...
  if(o.esPremioFidelidad)msg+=`🎁 *¡Esta alistada fue GRATIS por tu fidelidad!* Gracias por seguir rodando con Veloce 🚲\n\n`;
  else msg+=`${lineaFidelidad(o._clienteUuid)}\n\n`;
...
// ===== Reseñas de Google por WhatsApp =====
function buildMensajeReseña(o){
  return `Hola ${o.clienteNombre} 👋, ¡gracias por confiar tu *${o.bici.marca} ${o.bici.modelo}* a *Veloce Bicicletas*! 🚲\n\nSi quedaste contento con el servicio, nos ayudarías un montón dejándonos una reseña en Google (te toma menos de 1 minuto):\n${state.googleReviewUrl}\n\n¡Mil gracias por tu apoyo! 🙏`;
}
async function configurarReseñaGoogle(){
  const url=prompt('Pega el link para dejar reseña en Google.\n(En tu Perfil de Empresa de Google → "Pedir reseñas" → copiar enlace. Suele verse como https://g.page/r/.../review)',state.googleReviewUrl||'');
  if(url===null)return;
  const clean=url.trim();
  try{
    await window.db.setSetting('google_review_url',clean);
    state.googleReviewUrl=clean;
    toast(clean?'Link de reseñas guardado ✓':'Link de reseñas borrado','success');
    if(currentView==='caja')renderCaja();
    if(_ordenAbierta!=null&&document.getElementById('modal-orden').style.display==='block'){guardarRepEnMemoria(_ordenAbierta);aplicarEditsEnMemoria(_ordenAbierta);abrirOrden(_ordenAbierta);}
  }catch(err){toast('Error: '+err.message,'error')}
}

// ===== Historial de una bicicleta (todas sus órdenes) =====
function historialBiciHTML(o){
  if(!o._biciUuid)return '';
  const otras=state.ordenes.filter(x=>x._biciUuid===o._biciUuid&&x.id!==o.id).sort((a,b)=>new Date(b.creado)-new Date(a.creado));
  if(!otras.length)return '';
  const filas=otras.map(x=>{const tot=totalOrden(x);const reps=(x.reparaciones||[]).map(r=>r.desc).filter(Boolean).join(', ');return `<div class="hist-entry"><div style="display:flex;justify-content:space-between;gap:6px"><span style="font-size:12px;font-weight:500">#${x.id} · ${fmtDate(x.creado)}</span><span class="meta">${tot?'$ '+tot.toLocaleString('es-CO'):''}</span></div><div class="meta">${esc((x.tiposTrabajo||[]).join(', '))||'—'}</div>${reps?`<div class="meta">${esc(reps)}</div>`:''}</div>`}).join('');
  return `<details style="margin-top:10px"><summary style="cursor:pointer;font-size:13px;font-weight:500">🚲 Historial de esta bicicleta <span class="meta">(${otras.length} servicio${otras.length>1?'s':''} previo${otras.length>1?'s':''})</span></summary><div style="margin-top:6px">${filas}</div></details>`;
}
...
function renderCaja(){
  ...
  <div class="card"><div class="card-header"><h2>⭐ Reseñas de Google</h2><button class="btn btn-sm" onclick="configurarReseñaGoogle()">${state.googleReviewUrl?'Cambiar link':'Configurar link'}</button></div><div style="font-size:12px;color:#666">${state.googleReviewUrl?`Link activo. En una orden terminada usa <strong>"⭐ Pedir reseña"</strong> para enviarlo por WhatsApp.<div class="meta" style="margin-top:4px;word-break:break-all">${esc(state.googleReviewUrl)}</div>`:'Configura el link de tu Perfil de Empresa de Google para pedir reseñas a los clientes por WhatsApp al terminar una orden.'}</div></div>`;
}
